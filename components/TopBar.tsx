...
const SaveStatusIndicator: React.FC<{ status: TopBarProps['saveStatus'] }> = ({ status }) => {
    const statusMap = {
        encrypting: { text: 'Saving...', icon: <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> },
        synced: { text: 'Saved', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg> },
    ...
    return <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 min-w-[80px] justify-end">
        {icon}
        <span>{text}</span>
    </div>;
};
        ...
        <div className="flex items-center gap-4">
            {isEditing ? (
                 <>
                    ...
                    <button onClick={onCancel} className="px-4 py-1.5 text-sm font-semibold rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">Back</button>
                    {/* Save button is kept for manual triggering, though auto-save works too */}
